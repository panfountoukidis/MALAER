...
def df_reformat(df1, df2):
    
    '''
    This function adds, or removes data from df1, based on the common index values
    of both df1 and df2, in order to make df1 to have the same length with df2.
    It returns df1 with the new data.
    
    INPUTS:
    df1, df2: the two DataFrames
    
    OUTPUTS:
    df1_new: the DataFrame df1, with the new data
    
    NOTES:
    This function might not be correct, because we add the last data of df1,
    instead of a mean value or something similar.
    '''
    ...
    if len(df1) == len(df2):
        ...
        ## Get the common index values between df1 and df2 ##
        common_idxs = df1.index.intersection(df2.index)
        
        for i in range(len(common_idxs)):
            
            ## Get the common index value ##
            ct = common_idxs[i]
            
            ## Get the respective data ##
            data1 = df1.loc[ct]; data2 = df2.loc[ct]
            
            ## Proceed depending on the dimensions of the data ##
            if data1.ndim == 1:
                
                ct_lst1 = [ct]; data1 = data1.values
                
                ## Add the same data of df1, until data1 has the same length as data2 ##
                if data2.ndim == 1:
                    
                    if i == 0:
                        fdata1 = data1; fct_lst1 = ct_lst1
                    
                    else:
                        fdata1 = np.row_stack([fdata1, data1]); fct_lst1.extend(ct_lst1)
                
                else:
                    
                    tdata = data1
                    for j in range(len(data2)-1):
                        data1 = np.row_stack([data1, tdata])
                        ct_lst1.append(ct)
                    ...
                    if i == 0:
                        ...
                        fdata1 = np.row_stack([fdata1, data1])
                        fct_lst1.extend(ct_lst1)
            
            else:
                
                ct_lst1 = list(data1.index); data1 = data1.values
                
                if data2.ndim == 1:
                    
                    ## Add the first values of data1 in data1, until data1 has the
                    ## same length as data2
                    if i == 0:
                        fdata1 = data1[0]; fct_lst1 = ct_lst1[0]
                    
                    else:
                        fdata1 = np.row_stack([fdata1, data1[0]]); fct_lst1.extend(ct_lst1[0])
                
                else:
                    
                    ## Check if the two length values are equal or not ##
                    if len(data1) == len(data2):
                        ...
                        ## Keep the data1 up to the length of data2 ##
                        if len(data1) > len(data2):
                            
                            if i == 0:
                                fdata1 = data1[:len(data2)]; fct_lst1 = ct_lst1[:len(data2)]
                            
                            else:
                                fdata1 = np.row_stack([fdata1, data1[:len(data2)]])
                                fct_lst1.extend(ct_lst1[:len(data2)])
                        
                        ## Add the last values of data1 in data1, until data1 has the same
                        ## length as data2
                        else:
        ...
        df1_new = pd.DataFrame(fdata1, fct_lst1, df1.columns)
